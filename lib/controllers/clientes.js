...
async function updateCustomer(req, res, next) {
  try {
    const {clientId, cliente} = req.session_data

    const result = await clientesModel.update({id: clientId}, cliente)
    if (result.result === 'success') {
      ...
      return facResponse.respRequest(res, {statusCode: 400, textToShow: {result: 'fail', message: 'Client not updated: Not found'}})
